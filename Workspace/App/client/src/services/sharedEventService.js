...
export const getEventResponses = async (access_token, axiosJWT) => {
    try {
        const res = await axiosJWT.get(`/api/collab/responses`, {
            headers: { Authorization: `Bearer ${access_token}` },
        });
        return res.data;
    } catch (error) {
...
export const readNotification = async (access_token, inviteId, readType, axiosJWT) => {
    try {
        const res = await axiosJWT.put(`/api/collab/read/${inviteId}?readType=${readType}`, {},{
            headers: { Authorization: `Bearer ${access_token}` },
        });
        return res;
    } catch (error) {
